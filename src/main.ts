...
import { CompilerRegistry } from "./compilers/registry";
import { MarkdownCompiler } from "./compilers/markdown";
import { CanvasCompiler } from "./compilers/canvas";
import { ExcalidrawCompiler } from "./compilers/excalidraw";
import { SecretsWrapper } from "./secrets_wrapper";
import { DependencyResolver } from "./dependency_resolver";
import { ShareFrontMatter, CanvasData } from "./types";
...
export default class S3PublishPlugin extends Plugin {
	...
	async onload() {
		...
		const isExcalidrawActive = !!(this.app as any).plugins?.plugins?.["obsidian-excalidraw-plugin"];
		if (isExcalidrawActive) {
			this.registry.register(new ExcalidrawCompiler(this.app));
		}

		this.registry.register(new CanvasCompiler(this.app, this.registry));
		this.registry.register(new MarkdownCompiler(this.app, this.registry));
		...
		// Initial marker update and indexing
		this.app.workspace.onLayoutReady(async () => {
			await this.indexPublishedFiles(true);
			this.updateFileMarkers();
		});
	...
	async indexPublishedFiles(silent = false) {
		let indexedCount = 0;
		const knownPaths = new Set(this.settings.publishedFiles.map((p) => p.path));

		// Index Markdown Files
		const mdFiles = this.app.vault.getMarkdownFiles();
		for (const file of mdFiles) {
			const cache = this.app.metadataCache.getFileCache(file);
			if (cache && cache.frontmatter && cache.frontmatter["share_id"]) {
				const shareId = cache.frontmatter["share_id"] as string;
				let shareUrl = cache.frontmatter["share_url"] as string | undefined;

				if (!shareUrl) {
					shareUrl = this.publisher.getPublicUrl(shareId);
				}

				if (!knownPaths.has(file.path)) {
					this.settings.publishedFiles.push({
						path: file.path,
						shareId: shareId,
						url: shareUrl || "",
					});
		...
		for (const file of canvasFiles) {
			try {
				...
				if (json.share_id) {
					...
					let shareUrl = json["share_url"] as string | undefined;
				...
				console.error("Failed to parse canvas for indexing", e);
			}
		}
		...
		if (indexedCount > 0) {
			...
			this.updateFileMarkers();
			if (!silent) {
				new Notice(`Indexed ${indexedCount} previously published files.`);
			}
		}
	}
	...
	async publishNoteRecursively(file: TFile, visited: Set<string> = new Set()) {
		...
		const linksToVisit = this.dependencyResolver.getLinksToVisit(file);
		...
		for (const linkPath of linksToVisit) {
			...
			if (linkedFile instanceof TFile && (linkedFile.extension === "md" || linkedFile.extension === "canvas")) {
	...
	async publishNote(file: TFile, updateBacklinks: boolean = true) {
		...
		try {
			...
			await this.dependencyResolver.processFileAssets(file, assets, visited);

			// Extract top-level banner filename for passing to compile
			let bannerFilename = this.dependencyResolver.getBannerFilename(file);
			...
			if (!compiler) {
				new Notice(`No compiler found to publish ${file.name}`);
				return;
			}
			...
			// Only copy to clipboard if this is the top-level file being explicitly published
			if (updateBacklinks) {
				await navigator.clipboard.writeText(url);
				new Notice(`Published! Link copied:\n${url}`);
			} else {
				// For silently updated backlinks just show a tiny notice without overwriting clipboard
				new Notice(`Published dependency: ${file.basename}`);
			}
