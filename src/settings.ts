...
export interface IS3PublishPlugin extends Plugin {
	...
	indexPublishedFiles(silent?: boolean): Promise<void>;
}
...
export class S3PublisherSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		if (this.activeTab === "files") {
			void this.displayPublishedFiles(containerEl);
		} else {
	...
	async displayPublishedFiles(containerEl: HTMLElement): Promise<void> {
		new Setting(containerEl)
			.setName("Published files")
			.setHeading()
			.addButton((button) =>
				button
					.setButtonText("Refresh Index")
					.setTooltip("Scan your vault for previously published files and add them to this list")
					.onClick(async () => {
						button.setButtonText("Scanning...");
						button.setDisabled(true);
						await this.plugin.indexPublishedFiles();
						this.display(); // Re-render to show updated list
					})
			);

		const contentContainer = containerEl.createDiv();
		const scanningText = contentContainer.createEl("p", { text: "Scanning vault...", cls: "setting-item-description" });
		
		await this.plugin.indexPublishedFiles(true);
		scanningText.remove();

		const publishedList = this.plugin.settings.publishedFiles || [];

		if (publishedList.length === 0) {
			contentContainer.createEl("p", { text: "No files published yet." });
		} else {
			const table = contentContainer.createEl("table", { cls: "s3-publish-table" });
